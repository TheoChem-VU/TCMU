"""Module containing errors to distinguish between tcmu-specific errors and general python errors from other packages / scripts."""

from tcmu.log import loadbar

some_list = {"a": 1, "b": 2, "c": 3}
loadbar(some_list)


class TCMUError(Exception):
    """Base class for all errors in the TCMU package."""

    pass


class TCMUJobError(TCMUError):
...
class TCMUMoleculeError(TCMUError):
...
class TCMUCompDetailsError(TCMUError):
...
class TCMUMissingOptionalPackageError(TCMUError):
    """
    ...
    def __init__(self, package_name: str):
        msg = f"The optional package '{package_name}' is required for this TCMU functionality, but is not available. "
        if (extras_name := self.extras_install.get(package_name, None)) is not None:
            msg += f"It can be installed using the command: pip install 'tcmu[{extras_name}]'. "
