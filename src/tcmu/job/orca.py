...
import tcmu.spell_check as spell_check
from tcmu.errors import TCMUMoleculeError
        ...
        if ntasks and mem:
            if self._molecule is not None:
                ...
                if not hasattr(self, "natoms") or self.natoms is None:
                    raise TCMUMoleculeError("You set the molecule as a path and did not supply the number of atoms.")
                natoms = self.natoms
