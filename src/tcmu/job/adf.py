...
import tcmu.log as log
from tcmu import environment, formula, molecule, spell_check, constants, connect
from tcmu.data import basis_sets, cosmo, functionals
from tcmu.errors import TCMUCompDetailsError, TCMUJobError
...
class ADFJob(AMSJob):
    ...
    def basis_set(self, typ: str = "TZ2P", core: str = "None"):
        ...
        .. seealso::
            :mod:`tcmu.data.basis_sets` for an overview of the available basis-sets in ADF.
        """
    ...
    def SCF(self, iterations: int = 300, thresh: float = 1e-8):
        ...
        .. note::
            ...
            This is to prevent the job from being flagged as a failure when reading it using :mod:`tcmu.results`.
        """
    ...
    def functional(self, funtional_name: str, dispersion: str = ""):
        ...
        .. seealso::
            :mod:`tcmu.data.functionals` for an overview of the available functionals in ADF.
        """
        ...
        if functional == "SSB-D":
            raise TCMUCompDetailsError(section="Functional", message='There are two functionals called SSB-D, please use "GGA:SSB-D" or "MetaGGA:SSB-D".')

        if not functionals.get_functional(functional):
            raise TCMUCompDetailsError(section="Functional", message=f"XC-functional {functional} not found.")
        else:
    ...
    def solvent(self, name: Union[str, None] = None, eps: Union[float, None] = None, rad: Union[float, None] = None, use_klamt: bool = False, radii: Optional[Dict[str, float]] = None):
        ...
        .. seealso::
            :mod:`tcmu.data.cosmo` for an overview of the available solvent names and formulas.
        """
        if name:
...
class ADFFragmentJob(ADFJob):
    ...
    def add_fragment(self, mol: plams.Molecule, name: Union[None, str] = None, charge: int = 0, spin_polarization: int = 0):
        ...
        for child in self.child_jobs.values():
            if any((atom.symbol, atom.coords) == (myatom.symbol, myatom.coords) for atom in child._molecule for myatom in mol):
                raise TCMUJobError(job_class=self.__class__.__name__, message="The atoms in the new fragment are already present in the other fragments.")
    ...
    def guess_fragments(self):
        ...
        .. seealso::
            | :func:`tcmu.molecule.guess_fragments` for an explanation of the xyz-file format required to guess the fragments.
    ...
    def remove_virtuals(self, frag=None, subspecies=None, nremove=None):
        ...
        if nremove is None:
            ...
            if self._core.lower() != "none":
                raise TCMUJobError(job_class=self.__class__.__name__, message="Cannot guess number of virtual orbitals for calculations with a frozen core.")
            ...
            if self._basis_set.lower() not in [bs.lower() for bs in basis_sets._number_of_orbitals.keys()]:
                raise TCMUJobError(job_class=self.__class__.__name__, message=f"Cannot guess number of virtual orbitals for calculations with the {self._basis_set} basis-set.")
    ...
    def frag_occupations(self, frag=None, subspecies=None, alpha=None, beta=None):
        ...
        def _divide_electrons(n_elec, spin_pol=0, charge=0):
            ...
            if total_elec % 2 != spin_pol % 2:
                raise TCMUJobError(job_class=self.__class__.__name__, message=f"Got an {('even', 'odd')[total_elec % 2]} number of electrons ({total_elec}), but an {('even', 'odd')[spin_pol % 2]} spin-polarization ({spin_pol}), which is incompatible.")
            ...
            if a < 0 or b < 0:
                raise TCMUJobError(job_class=self.__class__.__name__, message=f"Got negative electrons for {total_elec} electrons with {spin_pol} spin polarization.")
...
class DensfJob(Job):
    ...
    def orbital(self, orbital: Union["pyfmo.orbitals.sfo.SFO", "pyfmo.orbitals.mo.MO"]):  # noqa: F821
        ...
        if isinstance(orbital, pyfmo.orbitals.objects.SFO):
            ...
            raise TCMUJobError(job_class=self.__class__.__name__, message=f"Unknown object {orbital} of type{type(orbital)}. It should be a pyfmo.orbitals.sfo.SFO or pyfmo.orbitals.mos.MO object.")
        ...
        if self.settings.ADFFile is None:
            ...
            raise TCMUJobError(job_class=self.__class__.__name__, message="RKF file that was previously set not the same as the one being set now. Please start a new job for each RKF file.")
