...
class WorkFlow:
    ...
    .. code-block::
        ...
        def optimize(molecule: str) -> "plams.Molecule":
            ...
            with tcmu.DFTBJob(use_slurm=False) as job:
    ...
    def __init__(self, server: connect.Server = None, delete_files: bool = True, preambles: List[str] = None, postambles: List[str] = None, sbatch: dict = None):
        self.server = server
        if server is None:
            self.server = tcmu.connect.Local()
        
        self.current_server = tcmu.connect.get_current_server()
    ...
    def _call_method(*args, **kwargs):
        ...
        self.cache_dir = os.path.join(platformdirs.user_cache_dir(appname="tcmu", appauthor="TheoCheMVU", ensure_exists=True), self.name, 'runs')
        self.results_dir = os.path.join(platformdirs.user_cache_dir(appname="tcmu", appauthor="TheoCheMVU", ensure_exists=True), self.name, 'results')
