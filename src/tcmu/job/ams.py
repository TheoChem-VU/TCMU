...
from tcmu import connect, read
from tcmu.job.generic import Job
from tcmu.job.postscripts import clean_workdir, write_converged_geoms
...
class AMSJob(Job):
    ...
    def _setup_job(self):
        ...
        print(os.path.join(self.workdir, 'ams.rkf'), os.path.exists(os.path.join(self.workdir, 'ams.rkf')))
    ...
    def restart(self, time_since_last_update: Union[float, datetime] = .5, enable: bool = True):
        '''
        Tell TCMU to restart this job from the latest checkpoint if possible.
