...
# These renaming imports are necessary to avoid circular imports (see the __init__.py file in the src/tcmu folder)
import tcmu.connect as connect
import tcmu.log as log
import tcmu.molecule as molecule
import tcmu.slurm as slurm
from tcmu.cache import cache
from tcmu.results.read import quick_status, get_timing
from tcmu.results.result import Result

__all__ = []  # Job must not be imported with 'from tcmu.job import *'
...
class Job:
    """This is the base Job class used to build more advanced classes such as :class:`AMSJob <tcmu.job.ams.AMSJob>` and :class:`ORCAJob <tcmu.job.orca.ORCAJob>`.
    The base class contains an empty :class:`Result <tcmu.results.result.Result>` object that holds the settings.
    ...
    def can_skip(self):
        ...
        .. note::
            This method works for the :class:`ADFJob <tcmu.job.adf.ADFJob>`, :class:`ADFFragmentJob <tcmu.job.adf.ADFFragmentJob>`,
            :class:`DFTBJob <tcmu.job.dftb.DFTBJob>` and :class:`ORCAJob <tcmu.job.orca.ORCAJob>` objects, but not
            yet for :class:`CRESTJob <tcmu.job.crest.CRESTJob>` and :class:`QCGJob <tcmu.job.crest.QCGJob>`. For the latter objects the job will always be rerun.
            This will be fixed in a later version of tcmu.
        """
        ...
        for server in self._servers:
            try:
                res = server.execute(f"tcmu read -s {j(server.pwd(), self.rundir, self.name)}")
    ...
    def run(self):
        ...
        if self.delete_on_fail:
            ...
            self.add_postamble(f"if [[ `tcmu read -s {self.workdir}` = FAILED || `tcmu read -s {self.workdir}` = UNKNOWN ]]; then rm -r {self.workdir}; fi;")
