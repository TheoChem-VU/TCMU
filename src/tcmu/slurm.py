...
from tcmu import cache, timed_cache
...
def squeue(server: connect.Server = connect.Local()) -> Result:
    ...
    Returns:
        : A :class:`Result <tcmu.results.result.Result>` object containing information about the calculation status:
    ...
    .. note::
        ...
        By default this function uses a timed cache (see :func:`timed_cache <tcmu.timed_cache>`) with a 3 second delay to lessen the load on HPC systems.
    """
    ret = Result()
...
def sbatch(runfile: str, server: connect.Server = connect.Local(), **options: dict) -> Result:
    ...
    Returns:
        : A :class:`Result <tcmu.results.result.Result>` object containing information about the newly submitted slurm job
